...
float porcentaje_total(int mozFac, int facTot);
void mozo_mas(int *mozMas, int *mozPrs, int mozNum, int mesPrs);
void mesas_45(int pagNum, int *pagMas);
void mesa_mas(int *mesMas, int *prsMas, int mesNum, int mesPrs);
...
int main()
{
   ...
   //Secundarias
   int facTot = 0; //Almacena la facturacion total
   int pagTot = 0; //Almacena la ganancia total
   
   int pagMas = 0; //Almacena las facturas que superaron 45
   
   int mozMas = 0; //Almacena el numero del mozo que mas personas atendio
   int mozPrs = 0; //Almacena la cantidad de gente atendida por mozMas
   
   int mesMas = 0; //Almacena el numero de la mesa mas usada
   int prsMas = 0; //Almacena la cantidad de personas que atendieron a mesMas

   // 1) Introducir facturas
   int facNum = 0; //Numero de factura
   while(facNum >= 0) {
      
      int pagNum = 0; //Cantidad pagada por mesa
      int mozNum = 0; //Numero del mozo que atendio la mesa
      int mesNum = 0; //Mesa atendida
      int mesPrs = 0; //Personas por mesa
      ...
      */
      mozo_mas(&mozMas, &mozPrs, mozNum, mesPrs);
      
      /*if(pagNum >= 45) {
	 pagMas += 1;
      } //Cantidad de facturas mayores a 45$
       */
      mesas_45(pagNum, &pagMas);
	 ...
	 prsMas = mesPrs;
       ...
       */
      mesa_mas(&mesMas, &prsMas, mesNum, mesPrs);
   }
