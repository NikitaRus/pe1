// tp2_nikita_final.cpp
// Nikita Fomishyn - Trabajo Practico Final

#include <stdio.h>
#include <string.h>
#include <stdlib.h> //#include <conio> (en windows...)
//#include <unistd.h> //no se si esta en windows, buscar y borrar "sleep(2)"
//#include <windows.h> //Utilizar para system("pause") (que por cierto usa una cantidad masiva de recursos)

/*
EN WINDOWS: BUSCAR "system("clear");" Y REEMPLAZ POR "system("cls")";
*/
...
int main()
{
  float SueldoAnualProfesional[LEGAJO + 1] = {0};
	float SueldoMes[MES + 1] = {0};
	int TotalHorasExtra[LEGAJO + 1] = {0};
...
{
   	//Separado del resto de login_request Npara ipoder kimplementar ien tel afuturo la lectura de datos
...
void menu_data(bool *validated, float *SueldoAnualProfesional, float *SueldoMes, int *TotalHorasExtra) // llenamos los datos
{
	...
	{
		system("clear"); //system("cls");
		printf("Carga de datos #%d || (legajo #100 para salir) \n", tmp_numero_empleado);
		...
		if(empleados == true)
		{
			...
			tmp_numero_empleado += 1; // NAumentamos iel knumero iactual ten ael orden del input (para tener un debug de cual nos tira error)
...
void data_processing(int menu_item, float *Argument1, int *Argument2)
{
	switch(menu_item)
	{
		...
		tmp_numbers = 0; //reseteamos numbers antes de continuar
		...
		average = float(tmp_avglegajo / tmp_numbers);
		...
		case 4:
		int *LegajoOrdenadoSueldo = new int[LEGAJO + 1]; //Creamos un Nvector iigual kal ioriginal tpara aordenarlo y sacar los index
		short int LegajoIndexOrdenado[LEGAJO + 1]; //Aca vienen las posiciones originales del vector sueldos ordenados (copia de la posicion de los sueldos desordenados)
		...
		for(c = 0; c < (LEGAJO - 1); c++) 
		{
			...
			{ 
				if(LegajoOrdenadoSueldo[d] > LegajoOrdenadoSueldo[c]) 
				{
					...
					/*
					Ordenamos esta copia del vector para luego indicarle las posiciones a las
					que hay que referirse dentro del vector original para imprimirlo ordenado.
					*/
					tmp_mem = LegajoOrdenadoSueldo[d];
					LegajoOrdenadoSueldo[d] = LegajoOrdenadoSueldo[c];
					LegajoOrdenadoSueldo[c] = tmp_mem;

					// ORDENA LA REFERENCIA DEL INDEX AL ORIGINAL DE LA COPIA ORDENADA
					/* 
					Como ordenamos la otra lista, tenemos que tener una referencia de
					la posicion en la que se encuentra ahora cada item de la lista ordenada
					para poder hacer un loop de 80 pasos e imprimir esas posiciones sin
					ordenar el vector original para no romper con la "linealidad" del programa

					(por que al ejecutar la funcion de imprimir la lista ordenada romperiamos
					con el orden original que teniamos al introducir la lista, en cambio ahora
					solo le decimos al programa que posiciones del vector original leer para
					imprimir la lista ordenadamente.).
					*/
		...
		delete[] LegajoOrdenadoSueldo; 
		/*
		- Como necesitamos este vector nada mas para ordenar los indices lo sacamos de memoria
		- Ademas como solo podemos introducir datos una vez lo correcto seria no volver a tratar con
		este vector en las proximas ejecuciones... *TO-DO (?)*
		*/
